...
Below, I quickly graph the first two models to see how the coefficients change when we only use complete cases. The `dotwhisker` package is handy for doing this, and the `dw_plot()` function in it does all the backwork in transforming our models into data frames, plotting them in `ggplot2` objects for us. Since it is `ggplot2`, I could then easily add on a vertical line. 
...
```{r  mod-comp}
dotwhisker::dwplot(list(mod1, mod2)) +
...
```{r imp-reg}
# impmod1 <- lm(imp_loginc ~ sex + race + workhrs,    # CAN"T USE THIS WITH MICE DATA
#               data = newimp)

impmod1 <- with(newimp, lm(imp_loginc ~ sex + race + workhrs))
...
```{r imp-sum}
summary(mice::pool(impmod1))

```

Also, `modelsummary` will do this for us. However, it won't give us goodness-of-fit info. 

```{r imp-compare}

mod4 <- lm(logincome ~ sex + race + workhrs,
           data = gss)
mod5 <- lm(imp_loginc ~ sex + race + workhrs,
           data = gss)
mod6 <- lm(logincome ~ sex + race + workhrs,
           data = gsscomp)
lastmods <- list("Orig Data" = mod4,
  ...
  # options(modelsummary_get = "glance") 
