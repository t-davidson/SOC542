...
ols_multi <- lm(hhinc_med_thou ~hh_comp + hs_grad + avg_family_size, 
                data = counties)
modelsummary(list(ols_bivar_comp, ols_bivar_educ, ols_bivar_fam, ols_multi),
...
Let's double-check to make sure we have no priors included here.
...
bayes_prior_r2 <- bayes_R2(bayes_prior_mod) %>% 
  as.data.frame() %>% mutate(model = "Priors")
bayes_flat_r2 <- bayes_R2(bayes_flat_mod) %>% 
  as.data.frame() %>% mutate(model = "Flat")
...
model_list <- list("OLS Bivariate: Computer" = ols_bivar_comp, 
                   "OLS Bivariate: Education" = ols_bivar_educ, 
                   "OLS Bivariate: Family Size" = ols_bivar_fam, 
                   "OLS Multivariate" = ols_multi, 
                   "Bayes - Flat Prior" = bayes_flat_mod, 
                   "Bayes - Specified Prior" = bayes_prior_mod)

coef_names <- c("hh_comp" = "% HH's w/Computer",
