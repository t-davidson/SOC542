...
Let's start with a uniform or "flat" prior. This assumes we are completely ignorant about the parameters of the model. We can specify this by setting `prior` and `prior_intercept = NULL`. There is also a prior on `sigma` but we will not modify it for now.
...
```{r trace}
plot(bayes_uniform, "trace", pars = "mom_iq") + labs(y= TeX("$\\beta_1$"), x = "Iteration")
```
...
## Strong priors
Let's see what happens if the priors are even stronger. Let's encode a prior that assumes a strong positive relationship between child and mother's IQ. Think for a moment about the implications of such an assuption. We can encode this by adjusting the prior on the coefficient. 
```{r strong-prior}
