...
(A true salsa dance of theory and methods! Hana/Norah/Luker would be proud.)
...
Our equation now is $$\hat{log_e(income)} = \alpha + \hat{\beta_{Work Experience}} + \hat{\beta_{(Work Experience)^2}} + \hat{\beta_{Work Hours}} + \hat{\beta_{Prestige}} + \hat{\beta_{Female}} + \hat{\beta_{White}} + \hat{\beta_{Other}} + \upsilon$$

Ignoring the rest of the terms, we can interpret the square term as $$\hat{log_e(income)} = \alpha + \hat{\beta_{Work Experience}} + \hat{\beta_{(Work Experience)^2}} + ... + \upsilon $$ 

For a person with 12 years work experience, we can then plug it in as: $$\hat{log_e(income)} = `r round(ols_worksq[["coefficients"]][["(Intercept)"]], 2)` + `r round(ols_worksq[["coefficients"]][["(Intercept)"]], 2)`*12 + `r round(ols_worksq[["coefficients"]][["(Intercept)"]], 2)`*(12^2) + ... + \upsilon $$ 

And simplify it down to: 
$$\hat{log_e(income)} = `r round(ols_worksq[["coefficients"]][["(Intercept)"]], 2)` + `r round(ols_worksq[["coefficients"]][["(Intercept)"]], 2)*12` + `r round(ols_worksq[["coefficients"]][["(Intercept)"]], 2)*(12^2)` + ... + \upsilon $$ 
$$\hat{log_e(income)} = `r round(ols_worksq[["coefficients"]][["(Intercept)"]], 2) +  round(ols_worksq[["coefficients"]][["(Intercept)"]], 2)*12 + round(ols_worksq[["coefficients"]][["(Intercept)"]], 2)*(12^2)` + ... + \upsilon $$ 
...
Note that the changes are very small, but still noticeable for some variables. Regardless, it is good practice to weight our data so that our estimates from the sample more accurately reflect the population. 
