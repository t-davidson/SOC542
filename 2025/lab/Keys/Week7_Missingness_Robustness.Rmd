...
gss <- gss %>%
  mutate(
    ...
    workhrs = as.numeric(case_when(
      hrs1 >= 89 ~ NA,       # Removing possible outliers
...
What do we do with the missing data? The approach should be driven by our research question and the variables it deems important. As Ricardo has said early on in the semester, we should use a subset of data that have no missing values across all variables of interest and the variables in our anticipated final model -- creating subsets accordingly (e.g., gss_avail). When missingness is random, imputation is a viable option, and the `mice` package provides an excellent set of tools for this purpose. However, if missingness is not random, it will introduce bias in our model and you should seek advice on more advanced techniques to appropriately handle the missing data. 
...
```{r bigimp}
bigimp <- gss %>% select(-wrkslf, -workexp, -income, -imp_loginc, -imp_income, -sex) %>% 
  mice(., m= 100, maxit = 5,
       seed = seed, printFlag=F)
impmod2 <- with(bigimp, lm(logincome ~ prestige + self_employed + race + female))
...
Heteroskedasticity refers to the situation in regression analysis where the variance of the errors is patterned across observations. This violates the ordinary least squares (OLS) assumption of homoscedasticity and can result in incorrect standard errors.

#### Performing an F-test for heteroskedasticity
```{r heteroskedasticity}
library(lmtest)

# Perform the Breusch-Pagan test on mod3
bptest_mod3 <- bptest(mod3)

# Output the test results
print(bptest_mod3)

```
In this example, we fail to reject the null hypothesis of homoskedasticity. But for next time...here is how you calculate robust standard errors -- the `lmtest` package.

#### Using robust standard errors
```{r robust standard errors}

# Calculate robust standard errors for mod3
robust_se <- coeftest(mod3, vcov = vcovHC(mod3, type = "HC1"))

# This can be done in model summary too!
modelsummary(mod3,
             estimate = "{estimate}",
             vcov = vcovHC(mod3, type = "HC1"),  
             stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001),
             title = "Comparison of Original and Robust Standard Errors",
             output = "huxtable")
