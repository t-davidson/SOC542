...
*Answer:* Square the lengths and sum to get the total variability of the data from its mean.
TSS quantifies the total variability in data by measuring the spread of all observations around the mean.
A higher TSS indicates greater overall variability available to be explained.
...
*Answer:* For a one-percentage point increase in poverty rate, we can expect a 0.338 increase in violent crimes per 1,000.
In other words, a 10% increase in poverty rate, we should expect to see 3.38 more violent crimes per 1,000.
...
*Answer:* Significant deviations from the normal distribution, such as skewness or the presence of outliers, may indicate issues with the model.
It could suggest that the model is not capturing all the patterns in the data, there might be unaccounted-for variables influencing the response, or that a linear model might not be the best fit for this data.
...
*Answer:* A random scatter implies that the model is doing well in accounting for the variance in the data.
Systematic patterns might indicate that the model is missing some explanatory variables or that the relationship between the variables is not purely linear.
