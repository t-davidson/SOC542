...
library("jtools") # For nice coefficient plots
library(rstanarm)
library(tidybayes)
...
gss <- gss %>%
  mutate(
      ...
      partyid %in% c(3,7) ~ "Other"  # Accounting for major and other political affiliations
    )
  )
...
```{r ols-base-model}
ols1 <- lm(income ~ age + prestige + workhrs + sex + race,
                   data = gss)
summary(ols1)
```
...
ols_dummy3 <- lm(income ~ age + prestige + workhrs + 
                   ...
                   sex_Female + 
                   degree_HS + degree_Assoc + degree_Bach + degree_Grad,
                 data = gss)
...
coef_plot <- plot_coefs(ols_dummy3, 
                        model.names = c("ols_dummy3"),
                        ci_level = 0.95) 
