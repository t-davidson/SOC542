...
# Course updates
## Homework
- Homework 3 grades released by end of week
- Homework 4 released after class, due next Friday, 4/17
    - Count outcomes
...
data <- read_csv("../../2022/slides/data/twitter_parties_2018.csv")#
data <- data %>%
...
# Poisson regression
## Modeling counts as Poisson processes
- Let's say the average number of visits to the doctor each year is 1.6.
...
$$P(1 \text{ visits a year}) = \frac{1.6^1 e^{-1.6}}{1!} = \frac{1.6e^{-1.6}}{1} \approx 0.32$$
...
- Coefficients can be interpreted as *multiplicative* changes after exponentiation
    - e.g., $e^{\beta_{seats\%}} = e^{0.013} \approx 1.013$. This implies a ~1.3% increase in tweets.
...
observed <- pois$model$tweet_count
left_right <- pois$model$left_right
...
ggplot(aes(x = value, group = variable, fill = variable), data = temp2) + geom_histogram(alpha = 0.8, bins = 10) + theme_classic() +  scale_fill_viridis_d(option = "cividis") + labs(fill = "Variable", y = "Density", x = "Tweet count")
```
...
$$y \sim NegativeBinomial(\lambda, \theta)$$
...
modelsummary(list("Poisson" = pois, "Negative binomial" = nb), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "RMSE", output = "latex", coef_omit = "country*", note = "Country FE omitted.", exponentiate = F, coef_map = c("seats_per" = "Seats %","left_right" = "Ideology [0-10]", "(Intercept)" = "Intercept"))
```
...
$$log(\lambda) = log(\mu) - log(\tau) = \beta_0 + \beta_1x$$
