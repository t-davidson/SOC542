...
$$log(\frac{P(y = \text{married})}{P(y = \text{never married})}) = \beta_{10} + \beta_{11}Age + \beta_{12}Sex  + \beta_{13}Income + \beta_{14}Educ$$
...
- Unlike standard logistic regression, where $e^{\beta}$ is an odds ratio, $e^{\beta_{11}}$ in a multinomial model is a \textbf{relative risk ratio}: the multiplicative change in $P(y = \text{married})/P(y = \text{never married})$ for a one-unit increase in the predictor.
...
# Multinomial logistic regression
## Predictions  
Setting `type = "probs"` returns a matrix of probabilities for each observation. Each element indicates $P(y_i = k)$.
