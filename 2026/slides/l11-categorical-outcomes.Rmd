...
# Course updates
- Homework 4 due Friday, 4/17 at 5pm
- Project preliminary results due next Friday, 4/24 at 5pm
    - See last week's slides for details
- Presentations in class on 5/7

# Course updates
- Labs
    - This week is final lab on lecture material
    - Remaining labs are project workshops
        - Attendance is still mandatory, but use the time to develop your projects and troubleshoot
...
- *Ordered* categorical outcomes
    - e.g. Likert scales: Strongly agree, agree, neither agree nor disagree, disagree, strongly disagree
...
# Multinomial logistic regression
- \textbf{Multinomial logistic regression} generalizes logistic regression to *unordered* categorical outcomes.
- For a set of $K$ outcomes, we can model the linear propensity for outcome $k$ using a linear model with $m$ predictors.

$$\lambda_k = \beta_{0k} + \beta_{1k}x_1 + ... + \beta_{mk}x_m$$
...
# Multinomial logistic regression
- Because the probabilities across categories must sum to one, one category will always produce the following equation:

$$\lambda_r = \beta_{0r} + \beta_{1r}x_1 + ... + \beta_{mr}x_m = 0 + 0x_1 + ... + 0x_m = 0 $$

- We therefore select a category to leave out as the *reference category*.
- Estimated coefficients can thus be interpreted as the relative log odds of each outcome compared to the reference category.
...
# Multinomial logistic regression
## Interpretation
- Each column is an equation for a specified outcome category compared to the *baseline* (Never married).
- For example, the first column represents the following equation:

$$log(\frac{P(y = \text{married})}{P(y = \text{never married})})$$

$$ = \beta_{10} + \beta_{11}Age + \beta_{12}Sex  + \beta_{13}Income + \beta_{14}Educ$$

# Multinomial logistic regression
## Interpretation
- $\beta_{11}$ indicates that a one-year increase in age is associated with a .092 change in the log odds of being married relative to never married.
- Unlike standard logistic regression, where $e^{\beta}$ is an odds ratio, $e^{\beta}$ in a multinomial model is a \textbf{relative risk ratio}: $e^{\beta_{11}} = 1.096$ is the multiplicative change in $P(y = \text{married})/P(y = \text{never married})$ for a one-unit increase in age.
    - The probability of being married *relative to never married* increases by 9.6% each year
...
preds <- predict(m1, gss %>% 
                     drop_na(age, sex, realrinc, educ, marital)
                 )
preds %>% head(20)
```
...
- Data imbalances make never/married the most likely categories.
    - Additional variables would likely help to improve accuracy for other categories.

# Multinomial logistic regression
## Predictions  
Setting `type = "probs"` returns a matrix of predicted probabilities for each observation. Each element indicates $P(y_i = k)$.
...
# Multinomial logistic regression
## Limitations
- Larger samples required compared to simpler models, particularly when categories are imbalanced.
...
# Ordinal logistic regression
## Estimation
- Each cutpoint represents the log-cumulative-odds that $y_i$ is less than or equal to some value $k$. These are analogous to *category-specific intercepts*.
...
# Ordinal logistic regression
## Estimation
- If we use the inverse link, we can go back from log-cumulative-odds to cumulative probabilities. The likelihood of $k$ is expressed as

$$p_k = P(y_i = k) = P(y_i \leq k) - P(y_i \leq k - 1)$$

- In the context of this example, we could express the likelihood of "Good" health as 

$$p_{\text{good}} = P(y_i = \text{good}) = P(y_i \leq \text{good}) - P(y_i \leq  \text{excellent})$$

# Ordinal logistic regression
## Estimation 
- Given this $K-1$ length vector of intercepts, $\alpha_{k \in K-1}$, we can use a linear model to predict the log-cumulative-odds that $y_i \leq k$ given a matrix of predictors $X$:

$$\phi_i = \beta X_{i}$$
$$log(\frac{P(y_i \leq k)}{1 - P(y_i \leq k)}) = \alpha_k - \phi_i$$

- Note: because $\phi_i$ is *subtracted*, a positive coefficient indicates that the predictor shifts the distribution toward higher outcome categories (e.g. toward poorer health). Some software and textbooks use the opposite sign convention.
...
modelsummary(list("Log odds" = m2, "Odds ratios" = m2), output = "kableExtra", exponentiate = c(F, T), coef_omit = "Excellent*|Good*|Fair*", gof_omit = "RMSE|BIC|edf", note = "Significance tests not provided by polr; no stars displayed.") %>% kableExtra::kable_styling(font_size = 8)
```

# Ordinal logistic regression
## Predictions
```{r polr-preds, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
preds2 <- predict(m2, gss %>% drop_na(health, age, sex,
                                      race, realrinc, educ))
preds2 %>% head(10)
```
...
probs2 <- predict(m2, type = "probs",
                  gss %>% 
                      drop_na(health, age, sex, 
                              race, realrinc, educ))
...
# Ordinal logistic regression
## Cutpoints
We can obtain the cumulative probability at each cutpoint by using the inverse logit function, $\frac{e^x}{1 + e^x}$. 
...
# Ordinal logistic regression
## Latent variables
The ordinal model implies a *latent variable* underlying the observed categories. We can approximate each observation's position on the cumulative probability scale by applying the inverse logit to the linear predictor ($\phi_i$), without subtracting the cutpoints. We can then observe where each observation falls between the cutpoints.
...
p + theme_minimal() + labs(y = "density", x = "z") + xlim(0,1) + geom_vline(xintercept = cut.probs[1], linetype = "dotted") + 
    geom_vline(xintercept = cut.probs[2], linetype = "dotted") + geom_vline(xintercept = cut.probs[3], linetype = "dotted")
...
- Similar to multinomial logistic regression
    - Larger samples required compared to simpler models.
    - Difficult to evaluate model fit.
    - Unstable if some variables perfectly predict category membership or have no overlap with certain categories.
    
# Ordinal logistic regression
## Proportional odds assumption
- Assumes the effect of each predictor on the log-odds of being at or below any given category is the same regardless of where the outcome is split (hence one set of coefficients).
- Additional tests are required to verify this is met.\footnote{\tiny See the \href{https://stats.oarc.ucla.edu/r/dae/ordinal-logistic-regression/}{UCLA stats blog} for details.}
