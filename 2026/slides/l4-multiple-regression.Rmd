...
# Multiple regression
## Example: Weath and education

$$Wealth_i = \beta_0 + \beta_1Educ + u$$

- Assume a strong, positive relationship between education and wealth (i.e. $\hat{\beta_1}$ is positive)
...
- Parental socioeconomic status (SES) is correlated with education and predicts wealth. 
    - The estimate of the effect of education is thus *biased* without taking parental SES into account.

# Drawing the DAG
```{r, echo=FALSE, out.width = "60%"}
D2 <- dagify(Wealth ~ Educ,
    Educ ~ pSES,
  Wealth ~ pSES,
  exposure = "Educ",
  outcome = "Wealth",
  coords = list(x = c(pSES = 1, Educ =1, Wealth = 3),
                y = c(pSES = 0, Educ = 2, Wealth = 1))
)
...
m.omit <- lm(y ~ x)
m.both <- lm(y ~ x + z)
m.omit$coefficients %>% round(2) %>% print()
m.both$coefficients %>% round(2) %>% print()
```
...
data <- data.frame(beta = c(coefs.omitted, coefs.complete),
                   type = c(rep("Omitted", sims), rep("Complete", sims)))
data %>%
  ggplot( aes(x=beta, fill=type)) +
    geom_density(alpha=0.6, position = 'identity') +
    theme_classic() +
    scale_fill_viridis_d(option="turbo") +
    geom_vline(xintercept = mean(coefs.omitted), linetype = "dotted", color = "black") +
    geom_vline(xintercept = mean(coefs.complete), linetype = "solid", color = "black") +
    annotate("text", x = 0.8, y = 1, label = TeX("$\\beta_1$"), vjust = -1, color = "white",size=5) +
    labs(fill="", x = TeX("$\\hat{\\beta_1}$"), y = "Density", caption = TeX("Distribution of $\\hat{\\beta_1}$ over 1000 simulations.")) + theme(legend.position ="top")
```
\tiny Dotted line shows estimate when $z$ is omitted; solid line when $z$ is included in the regression.
...
- We typically add control variables to mitigate omitted variable bias.
...
$$\sum_{i=1}^n (y_i-\beta_0 - \beta_1x_{1i} - \beta_2x_{2i} - ... - \beta_kx_{ki})^2 $$
...
# Multiple regression
## OLS in matrix form
- We can write the equation for multiple regression more compactly using matrix notation. Here $X$ is an $n$ by $k$ matrix of predictors and $\boldsymbol{\beta}$ is vector of coefficients with length $k$.


$$y = \beta_0 + \boldsymbol{\beta} X + u$$
...
# Multiple regression
## Model fit and $R^2$
- We define $R^2$ in the same way as a simple regression:
...
x2 <- rnorm(N,0,1)
z <- 0.5*x + rnorm(N,0,1)
y <- 0.5*x + -0.5*x2 + 0.05*z + rnorm(N, 0, 2)
```
...
data2 <- data.frame(se = c(se.omitted, se.complete),
                   type = c(rep("No Z", sims), rep("With Z", sims)))
data2 %>%
  ggplot(aes(x = se, fill=type)) +
    geom_density(alpha=0.4, position = 'identity') +
    theme_classic() +
    scale_fill_viridis_d(option="turbo") +
    labs(fill="", x = TeX("Standard error of $\\hat{\\beta_1}$"), y = "Density", caption = TeX("Distribution of standard error over 1000 simulations."))
```
...
# Variable selection\footnote{\tiny See Cinelli et al. 2022; Kohler et al. 2020; Keele et al. 2023.}
- It is conventional practice to include a wide array of potential confounders in a regression model as controls.
    - Sometimes derided as "kitchen sink" or "garbage can" regressions).
- But omitted variable bias, multicollinearity, and collider bias must be considered when deciding what to include.
- Domain knowledge and theory can guide model specifications, but these issues cannot be identified from the data alone.

# Variable selection
- "[A]uthors should not just list covariates that may affect the outcome but should disclose how they think those covariates and the exposure are causally related." \begin{tiny} Kohler et al. 2020, p. 12 \end{tiny}
...
# Variable selection
## Mediators
- *Mediators* are variables that partially explain the causal effect of $x$ on $y$ 
    - e.g. \textbf{first job} might mediate explain some of the effect of education on wealth.
- Controlling for a mediator will change the estimate of $x$ from the *total* effect to the *conditional effect*
...
- This is sometimes referred to as "blocking the backdoor path"
- This is related to OVB, which addresses bias in the estimation, rather than the causal structure.
...
# Bad control
## Causes of the independent variable can reduce precision\footnote{\tiny But we will see later in the semester how these can be helpful for causal indentification as instrumental variables.}
```{r, echo=FALSE, out.width = "60%"}
D2 <- dagify(Wealth ~ Educ,
  Educ ~ B,
  exposure = "Educ",
  outcome = "Wealth",
  coords = list(x = c(B = 1, Educ =1, Wealth = 3),
                y = c(B = 0, Educ = 2, Wealth = 1))
)
ggdag_status(D2, layout = "circle") +
  theme_dag() + theme(legend.position = "none")
```
...
# Neutral control
## Descendants of the independent variable are causally unrelated
```{r, echo=FALSE, out.width = "60%"}
D2 <- dagify(Wealth ~ Educ,
  N ~ Educ,
  exposure = "Educ",
  outcome = "Wealth",
  coords = list(x = c(N = 1, Educ =1, Wealth = 3),
                y = c(N = 0, Educ = 2, Wealth = 1))
)
ggdag_status(D2, layout = "circle") +
  theme_dag() + theme(legend.position = "none")
```
...
# Good control
## Independent causes of the outcome can increase precision
```{r, echo=FALSE, out.width = "60%"}
D2 <- dagify(Wealth ~ Educ,
  Wealth ~ G,
  exposure = "Educ",
  outcome = "Wealth",
  coords = list(x = c(G = 1, Educ =1, Wealth = 3),
                y = c(G = 0, Educ = 2, Wealth = 1))
)
ggdag_status(D2, layout = "circle") +
  theme_dag() + theme(legend.position = "none")
```
...
# Bad control
## Descendents of the outcome can induce bias
```{r, echo=FALSE, out.width = "60%"}
D2 <- dagify(Wealth ~ Educ,
  B ~ Wealth,
  exposure = "Educ",
...
# Variable selection
## Colliders
- $c$ is *caused by* $y$ *and* $x$.
- In this case, $c$ is a \textbf{collider} and controlling for $c$ can introduce bias.
...
```{r, echo=FALSE, out.width = "60%"}
D3 <- dagify(Wealth ~ Educ,
    C ~ Wealth,
  C ~ Educ,
  exposure = "Educ",
  outcome = "Wealth",
  coords = list(x = c(C = 1, Educ =1, Wealth = 3),
                y = c(C = 0, Educ = 2, Wealth = 1))
)
...
include_graphics("../../img/educ_wealth_dag.png")
```

# Exercise
## Drawing your DAG
1. Consider a relationship between two variables that you are interested in modeling
2. List some possible control variables 
3. Based on knowledge and theoretical considerations, draw a DAG
    - Either by hand or using \url{https://dagitty.net}
4. What kinds of good and bad controls do you observe?


# Multiple regression
## Interpreting multiple controls
- So far all of the specifications have focused on a single predictor $x$ and its relationship to the outcome $y$, net of controls
- It is common practice to give an interpretation to one or more, or even all, of the control variables
    - However, our focus is only on the identification of the main relationship, so these quantities may not be meaningful
- "Researchers should avoid providing any interpretation for estimates of control variables." \begin{tiny} Keele et al. 2023, p.12. \end{tiny}
