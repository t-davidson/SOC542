...
# Course updates
- Homework 3 released today, due next Friday 4/3
    - Binary outcomes
    - Interaction terms
    - Marginal effects
...
# Linear probability  model
## Definition
- The \textbf{linear probability model (LPM)} is estimates the *probability* a binary dependent variable is equal to one as a *linear* function of predictors.
...
lpm <- lm(shg ~ age + I(age^2) + caste + as.factor(village),
          data = data)
```

# Linear probability model
## Correctly estimating fixed-effects and standard errors
```{r dom-lpm-fe, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
library(fixest)
lpm.fe <- feols(shg ~ age + I(age^2) + caste | village,
                cluster = ~village, 
                data = data)
```
...
modelsummary(list("Model 1" = lpm, "Model 2" = lpm.fe), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|R2|RMSE|F|Log.Lik.", output = "latex", coef_omit = "as.factor*", notes = list("Village fixed-effects omitted."), 
...
coefs <- fixef(lpm.fe)$village %>% as.data.frame()
colnames(coefs) <- "fe"
...
ggplot(data = temp, aes(x = prop, y = fe)) + geom_text(aes(label=village), alpha = 0.8, position=position_jitter(width=0.05,height=0.02)) + theme_minimal() +
    labs(x = "Proportion SHG members", y = "Fixed-effect")
```
...
# Linear probability model
## Strengths
- Despite the limitations, the LPM has grown in popularity as an alternative to logistic regression
    - Simple to estimate
    - Easy to interpret
    - Prediction range often plausible or irrelevant
...
- Bayesian estimation does not converge on a single maximum likelihood estimate of $\hat{\beta}$, but produces a posterior distribution, $\hat{\beta}_{posterior}$
...
- We can get the \textbf{odds ratio} by exponentiating the coefficients
    - $OR(age) = exp(0.3) = 1.35$. A 1 year increase in age is associated with a 35% increase in the odds of SHG membership.
...
- The divide-by-4 rule provides a quick way to assess the effects of predictors in a logistic regression on the probability scale:
...
# Logistic regression
## Confidence intervals
The `confint` function in R allows us to calculate the correct confidence intervals (in this case for the effect of age). `confint.default` provides standard Wald confidence intervals. The intervals are exponentiated to get odds ratios.
...
# Logistic regression
## Model fit: Log-likelihood (Bayesian)
- The estimates using `predict` and `posterior_predict` are almost identical, but you should always use the full posterior distribution when computing any summary statistics from Bayesian models.
