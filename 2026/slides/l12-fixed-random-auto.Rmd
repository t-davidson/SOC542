...
# Course updates
- Preliminary results due Friday 4/24 at 5pm
    - One table with descriptive statistics on key variables
    - At least one descriptive figure
    - One regression table showing
        - Bivariate results
        - Multivariate results
    - At least one figure showing additional estimates (e.g. coefficients, predictions, marginal effects)
    - Draft write up of methodology and results
    - Submit PDF via email, 542 in subject line

# Course updates
- Presentations on 5/4
...
- Fixed effects
- Random effects
- Autocorrelation: space, time, and social structure
...
# Robust and clustered standard errors
## Intuition
- OLS coefficient estimates remain consistent under heteroskedasticity, but the *variance estimates* are inconsistent
    - This means standard errors, confidence intervals, and hypothesis tests are unreliable, even with large samples.
...
modelsummary(list("OLS" = m, "OLS (robust) " = m.r, "OLS (clustered)" = m.r.g),stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE|Log.Lik.|F|R2")
```
...
- $\gamma$ is a vector of fixed effect coefficients, one for each group.
...
- Considering data with a grouped structure, such as students nested within schools
    - Standard regression approaches imply \textbf{complete pooling} since all available data are used to estimate a population mean.
...
## Panel data
- GSS panel
    - Samples of 2016 and 2018 respondents were re-interviewed in 2020 (online)
...
# 2016, 2018, and 2020. Each respondent has 2 obs.
...
```{r fixest-cluster, echo = TRUE, mysize=TRUE, size='\\footnotesize', warning = F}
fe.ry.c <- feols(natcrime ~ sex + race +
                     age + polviews | region + year, 
                 cluster = ~ id, data = gss.new)
```

# Fixed effects
```{r table2b, echo = FALSE, mysize=TRUE, size='\\footnotesize'}
modelsummary(list("R-Y" = fe.ry, "R-Y FE (Clustered)" = fe.ry.c), stars = F, gof_omit = "AIC|BIC|RMSE|F|Log.Lik|R2 Within|Std.Errors", coef_omit = "sex2|age|race")
```
...
- Consider the case where we observe random variables $y$ and $x$, where observations belong to $j$ groups.
...
- Unlike fixed effects, where between-group variance is absorbed by the fixed effect, variables can be incorporated at different levels
...
# Random effects
## Estimation in R
The `lme4` package can be used to estimate random effects models in R using Maximum Likelihood or REML (the default). `lmer` function can fit a standard model; `glmer` generalizes to other link functions. The random part is specified in parentheses.
...
- For example, does the effect of sex on attitudes vary across regions?
...
We can modify the formula to include random slopes. The control argument is included due to estimation issues.\footnote{\tiny Warnings suggest potential problems with the model fit that require more detailed exploration.}
...
- One convention is to use a test to identify whether random effects should be included over fixed effects using a Hausman test, which evaluates whether covariates are correlated with the random effects, but this practice has been questioned (Bell and Jones 2019).
...
## Autocorrelation
- \textbf{Autocorrelation} refers to correlation between values of the same variable across observations
...
- Temporal autocorrelation is the most typical case, where successive measurements are correlated across time
...
gdp_df <- read.csv(url("https://fred.stlouisfed.org/graph/fredgraph.csv?id=GDP",
                       method = "libcurl"))
colnames(gdp_df) <- c("date", "value")
...
- Time series data can be decomposed into different components 
    ...
    - Seasonality represents regular periodic fluctuations
...
- AR(1) model: $y_t = \alpha + \beta y_{t-1} + \varepsilon_t$
...
- Time series analysis is a branch of statistics concerned with modeling temporal dynamics
...
- Data processing
    - e.g. De-trending and deseasonalizing time series variables
- Model specification
