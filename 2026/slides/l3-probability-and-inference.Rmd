...
- If the test has a false positive rate of 5\%, $P(+|C19-)P(C19-) = 0.05*(1-0.01)$
...
\textbf{"Bayesian inference is reallocation of credibility across possibilities"} - John Kruschke\footnote{\tiny Chapter 2 of Kruschke's 2015 book \textit{Doing Bayesian Data Analysis} provides an outline of his argument and is \href{https://scitechconnect.elsevier.com/wp-content/uploads/2015/05/DBA-Chapter-2.pdf}{available online}.}
...
- In this case, the posterior distribution is simple to calculate
    - $Beta(1,1) \rightarrow Beta(1+married,1+single) \rightarrow Beta(6,5)$
...
```{r bayes, out.width="50%", fig.align="center"}
...
```{r laplace, out.height="50%", fig.align="center"}
...
```{r fisher, out.height="50%", fig.align="center"}
...
hist(rbeta(1000, 6, 5), ylab = "density", xlab = "p", main = "Sampling from Beta(6,5)")
```
