...
R has several built-in datasets, one of which is `kidiq`. You can simply load it by running the following:
...
modelsummary(list("OLS" = model_ols),
             ...
             notes = "Notes: *** p < 0.001, ** p < 0.01, * p < 0.05")
```

The `modelsummary` function helps create a regression table that shows coefficient estimates, uncertainty measures (i.e., confidence interval), and statistical significance with star symbols. Specifically, it shows the result from the OLS regression, but it is not without limitations:
...
We can view and confirm the information about the priors we used by running the `prior_summary` command.
...
Thus, this weakly informative prior allows for moderate deviations from 0, effectively assuming "no effect" with 95% confidence.

```{r weak-prior}
bayes_weak_prior <- stan_glm(
  kid_score ~ mom_iq,
  data = kidiq,
  prior = normal(location = 0,   # location = mean
                 scale = 1),     # scale = sd 
  refresh = 0
) # `refresh = 0` to avoid printing out all of the estimation information we saw in the last example.

print(bayes_weak_prior)
```

```{r weak-compare-ols}
modelsummary(list("OLS" = model_ols, "Bayes Flat" = bayes_uniform, "Bayes Weak" = bayes_weak_prior),
             estimate = "{estimate}{stars} [{conf.low}, {conf.high}]",
             statistic = NULL,
             stars = c("***" = 0.001, "**" = 0.01, "*" = 0.05),
             notes = "Notes: *** p < 0.001, ** p < 0.01, * p < 0.05")
...
*Interlude: Prior Predictive Simulation*
...
## Prior predictive simulation with uniform prior
bayes_uniform_pp <- stan_glm(
  kid_score ~ mom_iq, data = kidiq,
  prior = NULL, # Note we are not providing any priors
  prior_PD = TRUE,
  refresh = 0
)
print(bayes_uniform_pp)

pp_check(bayes_uniform_pp) +
  theme_tidybayes()

## prior predictive simulation with a weakly informative prior
bayes_weak_pp <- stan_glm(
  kid_score ~ mom_iq, data = kidiq,
  prior = normal(location = 0,
                 scale = 1), # We now provide a standard normal distribution as a prior
  prior_PD = TRUE,
  refresh = 0
)
print(bayes_weak_pp)
...
Without seeing any real data, the prior predictive simulation with the uniform prior did not generate any meaningful estimates. This is because we are basically telling the model that *any* values are possible candidates. In contrast, the model with the weakly informative prior generates at least some estimates close to the actual IQ scores, but still they are very noisy.

Then what would happen if we provide data so the model can update their priors?

## Posterior predictive checks

We can evaluate the performance of our Bayes model by simulating data from the posterior distribution and compare it to the original data.
This is called *posterior predictive* check.

```{r posterior-check-weak}
pp_check(bayes_weak_prior) +
  ggtitle("Posterior Predictive Check: Weak Priors") +
  theme_tidybayes()
```
...
In this case:\
1. The posterior predictive distribution is slightly **left-skewed**, meaning the model underestimates some higher `kid_score` values.\
2. The distribution spread suggests that **uncertainty is well captured**, though extreme values are less frequent in simulated predictions compared to actual data.

**Reflection: What would happen if we never used observed data to build a model?**
...
## 3. Strong Prior (i.e., you have a strong theory)
...
Let's compare the coefficients across the four models.
...
# Additional code resources for the future:
...
## Alternative visualizations for  posterior predictive checks
