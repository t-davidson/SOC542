...
<!--
# Statistics review
The line of best fit is defined by the linear relationship between $x$ and $y$. We can use the `lm` function to find the intercept $\beta_0$ and slope $\beta_1$.
```{r, echo=FALSE, mysize=TRUE, size='\\footnotesize'}
best.fit <- lm("y ~ x", data = df)
round(best.fit$coefficients[1], 3)
round(best.fit$coefficients[2], 3)
```
-->
...
<!--- Slope can also be written in terms of correlation $\beta_1 = cor(x,y)\frac{\sigma(y)}{\sigma(x)}$ -->
...
<!-- Confirm this. Possible HW question is to generalize above sim to two variables-->
...
<!--
$$\tilde{\beta_1} = \sum_{i=1}^n\alpha_iy_i$$
where the weights $\alpha_i$ are a function of $x_i$.

- An estimator $\tilde{\beta_1}$ is conditionally unbiased if $E[\tilde{\beta_1}| x_1, x_2,..., x_n] = \beta_1$.

TODO: Return to this. See if there is a better way to explain it in the context of sampling distributions.
-->
...
# Comparing `lm` and `stan_glm`
We can also compare the standard deviations of the residuals, $\sigma$. The results are almost identical.
```{r, echo=TRUE, mysize=TRUE, size='\\footnotesize'}
sigma(model)
sigma(model2)
```

<!-- NOTE: sd(model2$residuals) provides a slightly different result to sigma(model2). Why?. See S&S on residual standard error. I think sigma is an estimate of the sampling distribution rather than the sample standard deviation.--->
