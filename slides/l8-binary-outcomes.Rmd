...
- The \textbf{linear probability model (LPM)} is used to model the *probability* of binary dependent variables as a *linear* function of predictors.
...
modelsummary(lpm, stars = TRUE, gof_omit = "AIC|BIC|R2|RMSE", output = "latex", coef_omit = "as.factor*", notes = list("Village fixed-effects omitted."), 
             coef_rename = c("age" = "Age", "I(age^2)" = "Age^2",
...
coefs <- as.numeric(lpm$coefficients)
coefs <- coefs[5:75]
idx <- unique(data$village)[-1]

v_props <- data %>% group_by(village) %>% summarize(n = n(), prop = sum(shg)/n)
prop <- v_props$prop[-1]
temp <- as.data.frame(cbind(prop, idx, coefs))

ggplot(data = temp, aes(x = prop, y = coefs)) + geom_text(aes(label=idx), alpha = 0.8, position=position_jitter(width=0.05,height=0.05)) + theme_minimal() +
    labs(x = "Proportion SHG members", y = "Village fixed-effect", caption = "Village 1 is reference category.")
```
...
- Heteroskedastic errors
    - Requires the use of heterosekasticity-robust standard errors\footnote{We will discuss this topic in more detail in Week 12.}
...
- The \textbf{logistic function} takes values in the range $[0,1]$ and maps them to the range $[-\infty, \infty]$
...
- The \textbf{inverse logistic function} reverses this transformation, mapping values back to the $[0,1]$ range:
...
- The model can be expressed in terms of $p_i$ using the inverse-logit function:
...
ggplot(data = df, aes(x=x, y=y)) + geom_point(alpha=0.5) + geom_smooth(method = "glm", se = F, method.args = list(family="binomial")) + theme_minimal()
```
...
- Binary variables follow a Bernoulli distribution,\footnote{Recall this is equivalent to $Binomial(1,p)$.} characterized by a single parameter, $p$, the probability of a success. Thus, $y \sim Bernoulli(p)$.
...
- For logistic regression, where $\beta$ is a vector of coefficients and $X$ is a matrix of predictors, the \textbf{likehood} is written as 
...
modelsummary(list("LPM"=lpm, "Logistic"=logit.mle), stars = TRUE, gof_omit = "AIC|BIC|R2|RMSE", output = "latex", coef_omit = "as.factor*", notes = list("Village fixed-effects omitted."), 
             coef_rename = c("age" = "Age", "I(age^2)" = "Age^2",
                             "castelow" = "Caste (Lower)"))
```
...
modelsummary(list("MLE"=logit.mle, "Bayes"=logit.bayes), stars = F, gof_omit = "AIC|BIC|ELPD s.e.|LOOIC s.e.|RMSE|F|LOOIC", output = "latex", coef_omit = "as.factor*", notes = list("Village fixed-effects omitted."), statistic = "conf.int", 
             coef_rename = c("age" = "Age", "I(age^2)" = "Age^2",
                             "castelow" = "Caste (Lower)"))
```
...
- We can get the \textbf{odds ratio} by exponentiating the coefficients
    ...
    - $OR(caste) = \frac{Odds(SHG=1|lower-caste)}{Odds(SHG=1|higher-caste)} = exp(0.315) = 1.37$. This implies that low caste residents are more likely to belong to SHGs (a 37% increase in the probability of SHG membership).
...
- There are several different approaches to construct \textbf{pseudo-$R^2$} statistics. These measures approximate an $R^2$ by ranging between 0 and 1, but do are not equivalent.\footnote{\tiny See this \href{https://stats.oarc.ucla.edu/other/mult-pkg/faq/general/faq-what-are-pseudo-r-squareds/}{blog post} for a discussion of several different measures.}
...
modelsummary(list("Logistic"=logit.mle, "Probit"=probit.mle), stars = TRUE, gof_omit = "AIC|BIC|RMSE", output = "latex", coef_omit = "as.factor*", notes = list("Village fixed-effects omitted."), 
             coef_rename = c("age" = "Age", "I(age^2)" = "Age^2",
                             "castelow" = "Caste (Lower)"))
```
