...
d <- cbind(X, X.m, X.g, X.g2)
colnames(d) <- c("Complete", "Missing", "Mean Imp.", "Random Imp.")
d2 <- melt(d)
colnames(d2) <- c("ID", "X", "x")
...
- The technique allows us to measure generalization error:
    ...
    3. Compare the predictive accuracy (e.g. mean squared error) across the two datasets.
        - Model generalizes well if $MSE(\hat{y}=f(X)) \approx MSE(\hat{y}=f(X'))$ and both MSE are low
        - Model overfits $MSE(\hat{y}=f(X)) << MSE(\hat{y}=f(X'))$
...
- In sociology, we typically estimate explanatory models where the primary goal is to estimate one or more $\hat{\beta}$'s.
- Prediction is typically a secondary concern and the best predictive models are not necessarily the best explanatory models.\footnote{\tiny See Mullainathan, Sendhil, and Jann Spiess. 2017. “Machine Learning: An Applied Econometric Approach.” Journal of Economic Perspectives 31 (2): 87–106. https://doi.org/10.1257/jep.31.2.87.
}
...
- There are several other diagnostics specific to Bayesian inference\footnote{\tiny See the \href{https://mc-stan.org/misc/warnings}{Stan documentation} for discussion of these issues and how to address them.}
