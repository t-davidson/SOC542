...
- Homework 2 released after class
    ...
    - Due next Tuesday (2/27) at 5pm via Github Classroom
    
# Paper propsals
- Plan for final paper
    - Relevant literature and research question(s)
        - Literature review does not need to be exhaustive!
    - Data
        - Outcome variable
        - Key predictors
    - Methodology
        - Regression approach
- Due 7/3 at 5pm (via email)
...
# Dummy variables
## Definitions
- A \textbf{dummy variable} is used to measure the difference between two possible states.
- Dummy variables are binary, taking a value of either zero or one.
- These values stand in for social categories of interest.
    - e.g. Male/female, employed/unemployed, liberal/conservative.
...
as.data.frame(cbind(x,y)) %>% group_by(x) %>% 
    summarize(mean = mean(y)) %>% as.matrix()
```
...
- The dummy variable also impacts the meaning of the intercept $\beta_0$, which is the average income for non-unionized workers.
...
```{r no-intercept, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
...
print(coefficients(u.reg)[2])
```

# Dummy variables
## Removing the reference category
- In the model with no intercept we get a separate coefficient for each category.
- The difference between these coefficients is equivalent to the dummy variable in the model with the intercept.
...
# Dummy variables
## Multiple dummy variables
This model of union wage returns includes a dummy variable for sex. What is the reference category and how can we interpret the intercept?
```{r multiple-dummies, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
u.reg2 <- lm(realrinc ~ union_dummy + sex, data = data)
print(u.reg2)
```

<!-- Commented out some more advanced material. Interesting but may get a little confusing.
# Dummy variables
...
- Consider the simple model, where $\beta_0$ represents the average income for non-unionized workers.
...
- In a Bayesian regression, the posterior distribution for $\beta_1$ can be obtained by taking the difference between the posterior distributions (McElreath refers to this as a *contrast*).
...
library(rstanarm)
data$union_dummy <- as.factor(data$union_dummy)
sm1 <- stan_glm(realrinc ~ union_dummy,  data = data, 
                family = "gaussian", chains =1 , refresh = 0)
sm2 <- stan_glm(realrinc ~ 0 + union_dummy,  data = data, 
                family = "gaussian", chains =1 , refresh = 0)
sm1$coefficients
sm2$coefficients
```
...
library(tidybayes)
posterior <- sm2 %>% spread_draws(union_dummynU, union_dummyU)
posterior$contrast <- posterior$union_dummyU - posterior$union_dummynU
head(posterior)
...
m.d <- lm(realrinc ~ union_dummy, data = data)
print(coefficients(m.d)[2]) # dummy coefficient
m.i <- lm(realrinc ~ 0 + union_dummy, data = data)
...
- The \textbf{exponential function} raises a *base* $b$ is raised to a power\footnote{\scriptsize Note how this differs from the power function, where we raise $x$ to a specified power. E.g. $power(x, 2) = x^2$} $x$.
...
modelsummary(list("Log x" = l1, 
                  "Log y" = l2, 
                  "Log-log" = l3,
                  "Log-log+" = l4), 
             gof_omit = "Log|AIC|BIC|RMSE", stars = TRUE, output = "latex")
```
...
modelsummary(list(m1, m2, m3, m4), gof_omit = "Log|AIC|BIC|F|RMSE", stars = TRUE, output = "latex")
```
...
# Lab
- Regression models using the General Social Survey
