...
# Course updates
- Homework 4 is cancelled
- Project update is extended until 4/26 at 5pm
...
# Categorical outcomes
## Intervals
- If a categorical variable is *ordered* there is some sense of an \textbf{interval} between categories such that each category can be positioned on a single dimension.
...
# Multinomial logistic regression
- \textbf{Multinomial logistic regression} models allow us to generalize logistic regression to *unordered* categorical outcomes.
...
- We jointly estimate a set of equations for each category.
...
<!-- Discuss independence of irrelevant attributes-->
...
# Multinomial logistic regression
## Estimation
- These models are more complex than other GLMs due to the estimation of multiple equations, so new packages are needed.
- Maximum likelihood models can be estimated using the `multinom` function from the `nnet` package\footnote{\tiny Other packages are available but require additional data manipulation before modeling. See \href{https://stats.oarc.ucla.edu/r/dae/multinomial-logistic-regression/}{this blog} for further discussion.}
...
- Like standard logistic regression $e^{\beta_{11}}$ allows us to interpret the coefficient as an odds ratio.
    - This is sometimes interpreted as the \textbf{relative risk ratio} of being married vs. never married.
...
# Ordinal logistic regression
- Approach to modeling ordinal data:
...
modelsummary(list("Log odds" = m2, "Odds ratios" = m2), output = "latex", exponentiate = c(F, T), coef_omit = "Excellent*|Good*|Fair*", gof_omit = "AIC|BIC|edf")
```
...
- Similar to multinomial logistic regression
    ...
    - Difficult to evaluate model fit
    - Unstable if some variables perfectly predict category membership or have no overlap with certain categories
- Assumption that the relationship between the predictors and each pair of outcomes is the same (hence one set of coefficients). This is known as the \textbf{proportional odds assumption}. Additional tests are required to verify this is met.\footnote{\tiny See the \href{https://stats.oarc.ucla.edu/r/dae/ordinal-logistic-regression/}{UCLA stats blog} for details.}
