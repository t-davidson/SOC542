...
modelsummary(list("LPM"=lpm, "Logistic"=logistic, "Odds-ratio"=logistic), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE|R2|Std.Errors|FE", exponentiate = c(F,F,T), output = "huxtable", coef_omit = "(Intercept)")
```
...
ggplot(aes(x = age, y = p, group = nonnative, color = as.factor(nonnative)),
    ...
    labs(y = "Predicted probability of SHG membership",
        x = "Age", color = "nonnative") + theme_classic()
```
\tiny Dotted black lines show linear fits to the predictions to help illustrate non-linearity. Neither line is truly linear, although the characteristic S-curve is only discernible for the steeper line.

# Predictions
## Using the `predictions` function
- These predictions can be obtained by using the  `predictions` function from `marginaleffects`.
...
ggplot(aes(x = age, y = estimate, group = nonnative, color = as.factor(nonnative)), data = preds) + geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "grey70", color = "black", alpha = 0.5, linetype = "dotted") +
...
modelsummary(list("Logistic 1"=logistic, "Logistic 2"=logistic2), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE", output = "huxtable")
```
...
- Like the example above, we may want to calculate the marginal effect of a predictor at specific values of other covariates.
    - e.g. What is the marginal effect of age by nativity at different values of age?

# Marginal effects
## Marginal effects at specified values: Nativity at age 25
...
round(ME.n$estimate[2] - ME.n$estimate[1],3)
```
...
ME.n2 <- slopes(logistic2,
                       newdata = datagrid(nonnative = c(0,1),
                                          age = c(65)))
round(ME.n2$estimate[2] - ME.n2$estimate[1],3)
```
...
```{r me-specified-nn-cme1, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
plot_slopes(logistic, variables = "age", condition = list("age", "nonnative" = 0)) + 
    theme_classic() + theme(legend.position = "none")
```
# Marginal effects
## Plotting conditional marginal effects using `plot_slopes`
```{r me-specified-nn-cme2, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
plot_slopes(logistic, variables = "age", condition = list("age", "nonnative")) + 
    theme_classic() + theme(legend.position = "none")
```
...
# Marginal effects
## Plotting conditional marginal effects using `plot_slopes`
The relationship is very different if we consider the model with the squared age term.
...
# Marginal effects
## Average marginal effects
This quantity can also be directly computed using `avg_slopes`.
```{r AME2, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
...
- We should also add the village-level fixed effects to account for spatial variation
...
modelsummary(list("Logistic 1"=logistic, "Logistic 2"=logistic2, "Logistic 3"=logistic3), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), coef_omit = "village", gof_omit = "AIC|BIC|RMSE|F|Num.Obs.", output = "huxtable")
```

# Marginal effects

```{r me-specified-age3, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
plot_slopes(logistic3, variable = "age", condition = c("age", "nonnative")) + theme_classic()
```
...
plot_slopes(bayes, variable = "age", condition = c("age", "nonnative")) + theme_minimal()
```
...
bayes.2 <- stan_glm(shg ~ age + I(age^2) + nonnative + age:nonnative + caste + educ + village, 
                    data = data, family = binomial(),
                    chains = 1,  refresh = 0)
```
...
plot_slopes(bayes.2, variable = "age", condition = c("age", "nonnative")) + theme_classic()
```
