...
modelsummary(list("LPM"=lpm, "Logistic"=logistic, "Odds-ratio"=logistic), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE|R2|Std.Errors|FE", exponentiate = c(F,F,T), output = "markdown", coef_omit = "(Intercept)")
```
...
- We can do this systematically by creating a new object containing every combination of predictors. The `predictions` function can then be used to obtain predicted values from the model.
...
modelsummary(list("Logistic 1"=logistic, "Logistic 2"=logistic2), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE", output = "markdown")
```
...
```{r AME, echo = TRUE, mysize=TRUE, size='\\scriptsize'}
AME <- summary(ME)
...
```{r AME-plot, echo = TRUE, mysize=TRUE, size='\\scriptsize'}
...
modelsummary(list("Logistic 1"=logistic, "Logistic 2"=logistic2, "Logistic 3"=logistic3), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), coef_omit = "village", gof_omit = "AIC|BIC|RMSE|F|Num.Obs.", output = "markdown")
```
...
```{r bayes.plot-final2, echo = FALSE, mysize=TRUE, size='\\scriptsize'}
plot_slopes(bayes.2, variable = "caste", condition = c("nonnative")) + theme_classic()
```

# Marginal effects
We can easily extend this approach to consider other variables.
```{r bayes.plot-final3, echo = FALSE, mysize=TRUE, size='\\scriptsize'}
plot_slopes(bayes.2, variable = "caste", condition = c("age", "nonnative")) + theme_classic()
```
