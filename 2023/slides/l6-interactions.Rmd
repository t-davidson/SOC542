...
m.r <- lm(y ~ x + z)
m <- lm(y ~ x + z + x:z)
modelsummary(list(m.r, m), gof_omit = "Log|AIC|BIC", stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), output = "latex")
```
...
gss <- haven::read_dta("../../2022/labs/lab-data/GSS2018.dta") %>%
    ...
    mutate(unions = ifelse(union == 1 | union == 3, 1, 0), # resp or resp and spouse unionized
           sex = ifelse(sex == 1, 1, 0),
           race = as.factor(race),
           realrinc = realrinc/1000)
gss$race <- recode_factor(gss$race, "1" = "White", "2" = "Black", "3" = "Other")

m00 <- lm(realrinc ~ sex, data = gss %>% drop_na(union))
m0 <- lm(realrinc ~ union, data = gss)
m <- lm(realrinc ~ sex + union, data = gss)
m.i <- lm(realrinc ~ sex + union + sex:union, data = gss)
modelsummary(list(m00, m0, m, m.i), gof_omit = "Log|AIC|BIC", stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), output = "latex")
```
...
modelsummary(list(m, m.i), gof_omit = "Log|AIC|BIC", stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), output = "latex")
```
...
modelsummary(list(m2), gof_omit=".*", statistic = NULL, stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), output = "latex")
```
...
modelsummary(list(m), gof_omit=".*", statistic = NULL, stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), output = "latex")
```
...
- In generalized linear models (GLMs), which will be out main focus after spring break, the coefficients often do not have clear interpretations on the outcome scale, making marginal effects evne more important for interpretation.\footnote{\footnotesize See the recommended reading, Mize 2019, for further discussion.}
