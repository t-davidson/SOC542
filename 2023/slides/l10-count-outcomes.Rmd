...
# Course updates
## Homework
- Homework 3 grades by end of week
- Homework 4 will be released later this week, due 4/14
    - Count outcomes
    - Categorical and ordered outcomes (next lecture)

# Course updates
## Projects
- Preliminary results due 4/24
    - Results from initial regression model(s)
        - Bivariate and multivariate
    - Must include at least one table and one figure
    - Write up of methodology and results
    - Recommended to use RMarkdown template (will be available on Github)
- Presentations on 5/1
...
- Includes number of tweets and engagements (likes, replies, retweets) from 2018
...
#ggplot(aes(x = tweet_count), data = data) + geom_hist(alpha = 0.5) + theme_classic()
hist(data$tweet_count, breaks = 10)
```
...
modelsummary(list("OLS" = ols.s, "OLS FE" = ols.fe, "OLS FE (Log)" = ols.fe.log), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE|Log.Lik.", output = "latex", coef_omit = "country*", note = "Country FE omitted.")
```
...
# Poisson regression
## Modeling counts as Poisson processes
- Let's say the average number of visits to doctor each week is 1.6.
...
X <- as.data.frame(x)
colnames(X) <- c("x")
ggplot(aes(x = x), data = X) + geom_histogram(bins = max(x), fill = "lightblue") + theme_classic() + labs(x = "Visits", y = "Count", caption = TeX("1000 draws from Poisson($\\lambda = 1.6$) using rpois"))  + scale_x_continuous(breaks=0:max(x))
```
...
modelsummary(list("OLS FE (Log)" = ols.fe.log, "Poisson" = pois), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE|F", output = "latex", coef_omit = "country*", note = "Country FE omitted.")
```
...
modelsummary(list("Poisson (Exponentiated)" = pois), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE", output = "latex", coef_omit = "country*", note = "Country FE omitted.", exponentiate = T)
```
...
ggplot(aes(x = value, group = variable, fill = variable), data = temp2) + geom_histogram(alpha = 0.8, bins = 50) + theme_classic() +  scale_fill_viridis_d(option = "cividis") + labs(color = "Variable", y = "Density", x = "Tweet count")
```
...
ggplot(aes(x = value, group = variable, fill = variable), data = X.tmp) + geom_histogram(bins = max(X$x2), alpha = 0.8) + theme_classic() + labs(x = "Visits", y = "Count", caption = TeX("Poisson($\\lambda = 1.6$) and NegBin($\\mu = 1.6$, $\\theta = 0.5$)"), fill = "DGP")  + scale_x_continuous(breaks=0:max(X$x2)) + scale_fill_viridis_d(option = "turbo")
```
...
modelsummary(list("Poisson" = pois, "Negative binomial" = nb), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE", output = "latex", coef_omit = "country*", note = "Country FE omitted. Exponentiated coefficients.", exponentiate = F)
```
...
modelsummary(list("Poisson" = m1, "Poisson (Log exposure)" = m2, "Poisson (Offset)" = m3), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE", output = "latex", exponentiate = F)
```

# Offsets
```{r offset-model-exp, echo = FALSE, mysize=TRUE, size='\\footnotesize'}
modelsummary(list("Poisson" = m1, "Poisson (Log exposure)" = m2, "Poisson (Offset)" = m3), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE", output = "latex", exponentiate = T, note = "Exponentiated coefficients.")
```
-->
...
modelsummary(list("NB" = nb.rt, "NB (Offset)" = nb.rt.o), stars = c("*" = 0.05, "**" = 0.01, "***" = 0.001), gof_omit = "AIC|BIC|RMSE", output = "latex", exponentiate = T, coef_omit = c("country*"))
```
...
ppc_dens_overlay(y = books,
                 yrep = posterior_predict(pois.m, draws = 50)) + xlim(0, 5) +
    labs(caption = 'Truncated to focus on lower predicted values', title = "Poisson")
```
...
pp_check(zip, ndraws = 50) + xlim(0, 5) +
