...
<!--TD: I think this example does too much. There is a lot to unpack!

Begin with a simple example w/o any modifications. e.g. use all defaults for function options except `prior = NULL` and `prior_intercept = NULL`

Introduce the other arguments below in following examples (let's not worry about family for now). Although this might make the models slightly less comparable (e.g. iter and chains may differ, I think it will be better to incrementally introduce some of the options)
...
bayes_uniform <- stan_glm(kid_score ~ mom_iq, data = kidiq,
                          ...
                          seed = 12345,  # Seed makes it reproducible by starting with same random number each time 
                          chains = 1,    # How many chains? 
...
# TD: Pedantic, but is there a way to get alpha to only apply to density
# Currently it also applies to the error bars and makes them a little hard to see...
```
...
pp_check(bayes_weak_prior, plotfun = "stat_2d", stat = c("mean", "sd")) + theme_tidybayes()
```
...
# Replication with Fewer Observations  

Above, we saw that there were only minuscule differences between the OLS the Bayesian models. Let's see what happens if we repeat the above exercises with fewer observations. 

```{r small_comp_mods}
small_kidiq <- slice_sample(kidiq, n = 50)

small_ols <- lm(kid_score ~ mom_iq, data = small_kidiq)
small_bayes_unif <- stan_glm(kid_score ~ mom_iq, data = kidiq,
                             family = gaussian(link = "identity"), 
                             prior = NULL,
                             prior_intercept = NULL,
                             seed = 12345, refresh = 0,
                             chains = 1, iter = 2000, warmup = 1000) 
small_bayes_weak <- stan_glm(kid_score ~ mom_iq, data = kidiq,
                             family = gaussian(link = "identity"),
                             prior = normal(location = 0, scale = 1), 
                             seed = 12345, refresh = 0)
small_bayes_strong <- stan_glm(kid_score ~ mom_iq, data = kidiq,
                               family = gaussian(link = "identity"),
                               prior = normal(location = 10, scale = 1),
                               seed = 12345, refresh = 0)

```

Now let's look at the coefficients with a graph: 
```{r small_comp_plot} 
s1 <- small_bayes_unif %>% gather_draws(mom_iq)
s2 <- small_bayes_weak %>% gather_draws(mom_iq)
s3 <- small_bayes_strong %>% gather_draws(mom_iq)
s1$model <- "Small Uniform"
s2$model <- "Small Weak"
s3$model <- "Small Strong"

e <- bind_rows(d, s1, s2, s3)

os <- broom::tidy(small_ols)
small_ols_coef <- os[[2, "estimate"]]
small_ols_se <- os[[2, "std.error"]]
...
e %>%
  ...
  geom_pointrange(aes(
    y = "Small OLS", x = small_ols_coef,
    xmin = small_ols_coef - 1.96 * small_ols_se,
    xmax = small_ols_coef + 1.96 * small_ols_se,
    )) +
  scale_fill_viridis_d() + theme_light() + 
  scale_y_discrete(limits = c("OLS", "Small OLS", "Uniform", "Small Uniform", 
                              "Weak", "Small Weak", "Strong", "Small Strong")) + 
  labs(y = "Model",
       x = "Coefficient Estimate") +
  theme(legend.position = "none",
        panel.grid.major.y = element_blank())
...
What do we see here? The Bayesian approach can help correct for a small sample size, even with uniform priors. 
