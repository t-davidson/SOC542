...
3.  In the `modelsummary()` function, we first include the list of our models. We could stop there and be fine. However, let's also make a few customizations: 
    ...
    -   Specify which test statistics we want. In this case, I want the standard error and t-value. I can put these into a vector so they appear on different lines. I also gave them prefixes so it's clear which is which.
    -   Omit any "goodness-of-fit" (or "`gof`") statistics aside from the number of observations (for all models) and the $r^2$.
...
Let's also create a model with specified priors. In this case, I approximated locations at 1, 1, and 0, with standard deviations of 1. The intercept in my previous multivariate model was around -16, so I'll put it there. - Feel free to play around with them, too. See what'll happen if you do a model with really large priors. Or one with very precise priors (small scales).

Because we have more than one independent variable, we need to save our priors in a list to add to the model.

```{r bayes-prior-model}
# Specifying priors
myprior_coef <- normal(location = c(1,1,0),
                       scale = c(1,1,1))
myprior_int <- normal(location = -16, scale = 2)
...
bayes_prior_r2 <- bayes_R2(bayes_prior_mod) %>% 
  as.data.frame() %>% mutate(model = "Specified Priors")
...
Finally, let's compare all three of our models we made today. We can use the `modelsummary()` function from earlier to put them all into a table.
