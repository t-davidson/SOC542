...
In other words, we run a regression of a variable, subtract the $R^2$ from 1, and use this to estimate how similar the variable is to the other variables included in the model. We'll do this for each independent variable to get a final table showing them all.
...
Fortunately, `modelsummary()` provides all three in the default. (But not for Bayesian Models.)
...
When it comes time to evaluate the model's performance itself, not just the goodness-of-fit, missingness, or multicollinearity of its variables, we can use cross-validation. There are multiple methods, but they all rely upon seeing how good the model is at predicting the value of data outside the model.
...
modelsummary(bmods, estimate = "{estimate} ({std.error})",
...
The results here show that model 1, compared to model1 2, decreases the log likelihood by 29.5, with a standard error of this difference of 8.8. Since 29.5 is much larger than 8.8 (an approximation of a t-test), we can say the second model is better.

We can also visually inspect our values to ensure there's no errant points flying around.
...
Similar to before, we see that the second model increased the log likelihood (AKA made it less negative), meaning it is better at predicting `logincome`.
