...
Let's start by creating three models of our target data using OLS methods. We'll come back to these later, but for now, just note that using OLS is an option, but not the option that fits either the data or the theoretical underpinnings of what you're trying to accomplish. 
...
So here we can say that each additional year of education increases the number of sexual partners a person had in the past year. The difference in the log of expected counts is expected to increase by .016 for each year of education. 
...
As an aside, notice how the Log Likelihood is much smaller in the Poisson model than in the OLS model. 

### Incidence Rate Ratios

For those of us who prefer odds ratios, we can create incidence rate ratios, which can be interpreted similarly. 
...
So here we see that each year of education increases the rate ratio of sexual partners by a factor of 1.015, or a 1.5% increase, and each year of age (ignoring the square term for now) decreases it by a factor of .027, or a 2.7% increase ($1.027-1 = .027$). 
...
*Remember that these "rate ratios" are ratios of one category's rate (e.g. female, widows) to the reference group's rate (e.g. male, never-married).*
...
Above, we have a table with 10 predictions (5 levels of marital * 2 levels of educ) for the number of sex partners. 
...
Above, we see our model did a pretty good job at predicting partners, although there still appears to be some unmodeled variation in later years. 
...
```{r sibs-overdisp}
ggplot(gss, aes(x = sibs)) + geom_density()
table(gss$sibs)
```
...
We see similar output here as we did in our Poisson model above. Note the Theta parameter at the bottom. This is the measure of dispersion in the model. Larger thetas provide better justification for using negative binomial instead of Poisson. 
...
So each additional year of age increases the incidence rate by a factor of 1.005 and each additional year of education decreases it by about 6%. Additionally, women tend to have an incidence rate that is about 10% higher than men's. 
...
The waay that `modelsummary` extracts coefficients tags each of the terms separately for the zero-inflated and count portions of the model, which also means you will have separate rows if you have multiple models. 
...
Another option is to use the `coef_map` option in `modelsummary::modelsummary()` to rename the coefficients. You can also rename OLS coefficients to share names and lines. 
