...
So instead of multiplying `r round(ols1[["coefficients"]][["sexMale"]],2)` by some abstract value of `sex`, we multiply it times 1 if the person is male, and by 0 if they are male. Essentially, we're adjusting the intercept term, not the slope. 
...
4. `sexFemale`: A person who is `Male` can expect to make $`r round(ols1[["coefficients"]][["sexMale"]],2)` compared to a person who is `Female`. 
...
modelsummary(list("Original" = ols_inc, "Inc/1k" = ols_thou, 
             ...
             output = "huxtable",stars = T, gof_omit = "F|RMSE|Log|IC")
...
ols_work <- lm(logincome ~ workexp + workhrs + sex,
                   data = gss)
ols_worksq <- lm(logincome ~ worksq + workhrs + sex,
                   data = gss)
...
modelsummary(list(ols_age, ols_work, ols_worksq, ols_work_worksq),
             output = "huxtable", 
...
func_wrkexp2 <- function(workexp) coef_workexp * workexp + coef_worksq * workexp^2

ggplot() + 
  stat_function(fun = func_wrkexp2, linewidth = 1) +
  scale_x_continuous(name = "Work Experience (Years Since Age 18)", 
                     limits = c(0,70)) +
  labs(y = "log(Income)", title = "Effects of Work Experience on Income",
       caption = "Holding constant work hours and sex") +
