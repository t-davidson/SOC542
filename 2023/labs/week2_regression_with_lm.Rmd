...
if(!require(modelsummary))          # If modelsummary package isn't intalled...
...
if(!require(ggExtra))          # If cowplot package isn't intalled...
...
We're then going to extract four variables that measure feelings toward the two Presidential and two Vice Presidential candiates. These are called Feeling Thermometers. From the questionnaire: 
...
To show the t-value, we change the command to include `statistic = 'statistic'`. 
```{r}
modelsummary(mod1, statistic = 'statistic', stars = T,
             gof_omit =  'DF|Deviance|RMSE|AIC|BIC|Log')
```
...
To see everything together, we need to do some more advanced work. Feel free to move around the pieces inside the curly brackets (`{}`), which tell modelsummary where to put each piece. 
...
We'll play around more with modelsummary later in the semester, and rarely do we need to include *all* of this info at once, so no need to remember it. 
...
Below are code shells; take a few minutes to plug in the appropriate variables and compare. 
