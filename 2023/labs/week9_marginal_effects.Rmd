...
```{r predggplot}
incpreds <- predictions(increg3, by = c("selfemp", "age", "sex"))
head(incpreds)
...
modelsummary(mfx, stars = T,
             ...
             gof_omit = "F|RMSE|IC|Log|R", output = "flextable") %>% autofit()
```
...
Below, I construct the marginal effects where education is set at 12 years. The equation is the same as above, `slopes(model), newdata = datagrid()`,  but with an argument into which we input our conditions.

```{r cme}
slopes(unemp1, newdata = datagrid(educ = 12))
```
...
plot_slopes(unemp1, variables = "logincome", condition = c("educ")) + 
  geom_hline(yintercept = 0, linetype = "dotted") + theme_light()
...
```{r cmeplot-cat}
plot_cme(unemp1, variables = "age", condition = c("selfemp","sex")) + theme_minimal()
```
...
Lastly, if we include a categorical variable in the `variables = VAR` argument, `plot_slopes()` will show the effects differently across them.  
