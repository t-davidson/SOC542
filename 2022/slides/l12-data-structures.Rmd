...
gss <- read_dta("data/gss2020panel_r1a.dta") %>% zap_labels()
...
```{r robust-standard-errors, echo = FALSE, mysize=TRUE, size='\\footnotesize'}
library(sandwich)
library(lmtest)
...
modelsummary(list("OLS" = m, "OLS (robust) " = m.r, "OLS (clustered)" = m.r.g),stars = TRUE, gof_omit = "AIC|BIC|RMSE|F|R2") # TODO: Add
```
...
modelsummary(list("Pooled" = ols, "Region FE" = fe.r, "Year FE" = fe.y, "Both FE" = fe.ry), stars = TRUE, gof_omit = "AIC|BIC|RMSE|F|R2|Std.Errors")
```
...
# Random effects
## Estimating random coefficient models
We can easily modify the formula to include random slopes. In this case, we allow the slope of race to vary according to the region. The control argument is included due to estimation issues.\footnote{\tiny Warnings suggest potential problems with the model fit that require more detailed exploration.}
...
modelsummary(list("Region RE" = re.r.logit, "Region RE & Race RC" = rc.logit), exponentiate = TRUE, stars = TRUE, gof_omit = "AIC|BIC|RMSE", coef_omit = "cor_*|sd_*")
```

# Random effects
```{r table4b, echo = FALSE, mysize=TRUE, size='\\footnotesize'}
modelsummary(list("Region RE" = re.r.logit, "Region RE & Race RC" = rc.logit), exponentiate = TRUE, stars = TRUE, gof_omit = "AIC|BIC|RMSE|Num.Obs|Log.Lik.", coef_omit = "race*|age|polviews*")
```
...
include_graphics("../../../example_plots/irr_populism_final.png")
```

# Example: Populism and social media engagement
```{r, out.width="70%",out.height="70%", fig.align="center"}
include_graphics("../../../example_plots/year_slopes_poppa_fb_likes.png")
```

# Example: Populism and social media engagement
```{r, out.width="70%",out.height="70%", fig.align="center"}
include_graphics("../../../example_plots/country_slopes_poppa_fb_likes.png")
```

# Example: Populism and social media engagement
```{r, out.width="70%",out.height="70%", fig.align="center"}
include_graphics("../../../example_plots/heatmap_slopes_popppa_fb_likes.png")
```

# Example: Populism and social media engagement
```{r, out.width="70%",out.height="70%", fig.align="center"}
include_graphics("../../../example_plots/country_predictions_poppa_likes_rs_cy_full.png")
```

# Example: Populism and social media engagement
```{r, out.width="70%",out.height="70%", fig.align="center"}
include_graphics("../../../example_plots/country_predictions_poppa_likes_rs_cy_subset.png")
```
...
```{r time-series-1, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
library(timeSeries)
t <- read.csv("data/multiTimeline.csv")
```
...
# Space, time and social structure
## Lagged variable specifications

# Space, time and social structure
## Unit roots and problematic inferences

# Space, time and social structure
# General Error Correction Model (GECM)
-->
